(function (factory) {
    ...
    function RadioButtons(props) {
        ...
        return (_("fieldset", { class: classname(b$g(), className) },
            label ? (_("legend", { class: b$g('label', { hidden: props.hideLabel }) }, label)) : '',
            _("ul", { class: b$g('items') }, buttons.map(function (item) {
                return (_(RadioButton, { key: item.value, class: b$g('item'), selected: item.value === selectedValue, title: item.title, text: item.text, value: item.value, onClick: handleSelect }));
            }))));
    }
    ...
    ***************************************************************************** */
    ...
    var isSsr$1 = typeof window === 'undefined';
    ...
    function GpuNavigator() {
        ...
        if (!isSsr$1 && !navigator.gpu) {
            return (_(WarningMessage, null, i18n('WebGPU is not supported.')));
        }
        if (hasAdapter === null) {
            return (_(WarningMessage, null, i18n('GPU Adapter is not found.')));
        }
        return hasAdapter ? (_("div", { class: b$c() },
            _("div", { class: b$c('controls') },
                _(RadioButtons, { label: "Power preference:", buttons: buttons, onSelect: onSelect })),
            _(TreeList, { title: "Adapter", data: data }))) : (_(ValueInProgress, null));
    }
    ...
    function List(props) {
        ...
        var className = classname(props.class, b$b());
    ...
    function WebGLContext() {
        ...
        items = items.filter(function (item) { return Boolean(item[1]); });
        return (_(List, { title: "WebGL context", items: items }));
    }
    ...
    function Cube3d() {
        return (_("div", { class: b() },
            _("div", { class: b('container') },
                _("div", { class: b('side', { back: true }) }),
                _("div", { class: b('side', { left: true }) }),
                _("div", { class: b('side', { right: true }) }),
                _("div", { class: b('side', { top: true }) }),
                _("div", { class: b('side', { bottom: true }) }),
                _("div", { class: b('side', { front: true }) }))));
    }
