(function (factory) {
    ...
    ***************************************************************************** */
    ...
    var b$l = block('section');
    function Section(props) {
        if (!props.children) {
            return null;
        }
        var className = classname(b$l(), props.class);
        return props.name ? (_("section", { class: className },
            _("h2", { class: b$l('name') }, props.name),
            _("div", { class: b$l('body') }, props.children))) : (_("div", { class: b$l() }, props.children));
    }

    var b$k = block('description-list');
    function DescriptionList(props) {
        var filteredItems = props.items
            ...
            item[0]; var details = item[1];
            return typeof details !== 'undefined' && details !== '';
        })
            .map(function (item) {
            var term = item[0], details = item[1];
            return [term, typeof details === 'boolean' ? String(details) : details];
        });
        var className = classname(props.class, b$k());
        return filteredItems.length ? (_(Section, { name: props.title },
            _("dl", { class: className }, filteredItems.map(function (item) {
                var name = item[0], value = item[1];
                return (_("div", { key: name, class: b$k('item') },
                    _("dt", { class: b$k('term') }, name),
                    _("dd", { class: b$k('details') }, value)));
            })))) : null;
    }
    ...
    var b$j = block('warning-message');
    function WarningMessage(props) {
        return (_("span", { class: b$j({ theme: props.theme }) },
            "\u26A0\uFE0F ",
            props.children));
    }

    var b$i = block('hdr-label');
    function HdrLabel(props) {
        return (_("span", { class: b$i({ enabled: props.enabled }) }, "HDR"));
    }

    var b$h = block('color-space-item');
    function ColorSpaceItem(props) {
        var name = props.name, supported = props.supported, type = props.type;
        return (_("li", { class: b$h({ type: type, supported: supported }) },
            _("span", { class: b$h('checked') }, supported ? '✓' : '✗'),
            " ",
            _("span", { class: b$h('color') }, name)));
    }
    ...
    var b$g = block('color-space-list');
    function ColorSpaceList(props) {
        ...
        return (_("ul", { class: b$g() }, items.map(function (item) {
            return (_(ColorSpaceItem, __assign({}, item)));
        })));
    }

    var b$f = block('screen-item');
    function ScreenItem(props) {
        ...
        var items = [
            ...
            [(_(HdrLabel, { enabled: Boolean(isHdr) })), getChecked(Boolean(isHdr))],
            [i18n('Color spaces'), (_(ColorSpaceList, { items: colorSpaces }))],
        ...
        return (_("div", { class: b$f() },
            label ? (_("div", { class: b$f('label') }, label)) : '',
            _(DescriptionList, { items: items }),
            isExtended === true ? (_(WarningMessage, null, i18n('Additional monitor detected'))) : ''));
    }

    var b$e = block('button');
    function Button(props) {
        var disabled = props.disabled, size = props.size, theme = props.theme, onClick = props.onClick, title = props.title;
        var className = classname(props.class, b$e({ theme: theme, size: size, disabled: disabled }));
    ...
    var b$d = block('spinner');
    function Spinner(props) {
        return (_("div", { class: b$d({ size: props.size }) }));
    }

    var b$c = block('refresh-rate');
    function RefreshRate() {
        ...
        return value ? (_("span", { class: b$c() },
            value.toFixed(3),
            " ",
            i18n('Hz'))) : (_(Spinner, { size: "s" }));
    }

    var b$b = block('page-title');
    function PageTitle(props) {
        return (_("h1", { class: b$b() }, props.children));
    }

    var b$a = block('screen-list');
    function ScreenList() {
        var forceUpdate = useForceUpdate();
        ...
        var additionalItems = [
            RefreshRateController.hasSupport() ? [i18n('Refresh rate'), (_(RefreshRate, null))] : ['', ''],
        ...
        return (_("div", { class: b$a() },
            _(PageTitle, null, screenInfoData.screens.length === 1 ? i18n('Screen') : i18n('Screens')),
            !screenInfo.isDenied && screenInfo.needUserActivity ? (_("div", { class: b$a('specify') },
                _(Button, { theme: "red", size: "s", onClick: handleClick }, i18n('Specify')))) : '',
            screenInfoData.screens.map(function (item) {
                return (_(ScreenItem, __assign({}, item)));
            }),
            _(DescriptionList, { title: i18n('Additionally'), items: additionalItems })));
    }
    ...
    function ScreenPage() {
        var items = [
            [(_(Link, { href: getPagePath('test-dead-pixels') }, i18n('Test dead pixels')))],
            [(_(ExtLink, { target: "_blank", href: "https://vvideo.github.io/hdcp/index.html" }, i18n('Check HDCP version')))],
        ];
        return (_(Page, null,
            _("div", null,
                _(ScreenList, null),
                _(Section, { name: i18n('Tests') },
