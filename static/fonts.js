(function (factory) {
    ...
    ***************************************************************************** */
    ...
    function Checkbox(props) {
        ...
        var className = classname(props.class, b$h({ checked: checked, theme: props.theme, focus: focus }));
        y(function () {
            if (!ref.current) {
                return;
            }
        ...
        return (_("label", { title: props.title, class: className, onClick: handleClick },
            _("input", { type: "checkbox", ref: ref, class: b$h('input'), checked: checked }),
            props.label));
    }

    var b$g = block('input');
    function Input(props) {
        var className = classname(b$g(), props.class);
    ...
    var b$e = block('font-item');
    function FontItem(props) {
        var _a = h(false), opened = _a[0], setOpened = _a[1];
        var handleClick = q(function () {
            setOpened(!opened);
        }, [opened]);
        return (_("div", { class: b$e({ opened: opened }) },
            _("div", { class: b$e('value') },
                _("span", { class: b$e('switcher'), onClick: handleClick }, props.fullName),
                opened ? (_("div", { class: b$e('content') },
                    _("div", { class: b$e('family') },
                        i18n('Family'),
                        ": ",
                        props.family),
                    _("div", { class: b$e('style') },
                        i18n('Style'),
                        ": ",
                        props.style))) : '')));
    }

    var b$d = block('font-list');
    function FontList(props) {
        var items = props.items;
        return (_("div", { class: b$d() }, items.map(function (item, i) {
            return (_(FontItem, { key: item.fullName, family: item.family, fullName: item.fullName, style: item.style, num: i + 1 }));
        })));
    }

    var b$c = block('font-family-item');
    function FontFamilyItem(props) {
        ...
        return (_("div", { class: b$c({ opened: opened }) },
            _("div", { class: b$c('value') },
                _("span", { class: b$c('switcher'), onClick: handleClick }, props.name),
                opened ? (_("div", { class: b$c('items') },
                    _(FontList, { items: props.items }))) : '')));
    }

    var b$b = block('font-list-grouped');
    function FontListGrouped(props) {
        var items = props.items;
        var groupedItems = groupByFamily(items);
        return (_("div", { class: b$b() }, groupedItems.map(function (item, i) {
            return (_(FontFamilyItem, { key: item.name, name: item.name, items: item.items, num: i + 1 }));
        })));
    }
    ...
    function Fonts() {
        if (!isSsr && !window.queryLocalFonts) {
            return (_(WarningMessage, null, i18n('Local Font Access API is not supported.')));
        }
        ...
        return (_("div", { class: b$a() },
            _("div", { class: b$a('controls') },
                fonts ? '' : (_(Button, { theme: "active", onClick: handleButtonClick }, i18n('Request fonts'))),
                fonts ? (_(Input, { class: b$a('filter'), placeholder: i18n('Filter'), value: "", onChange: handleChange })) : '',
                fonts ? (_("div", { class: b$a('group-by-family') },
                    _(Checkbox, { onClick: handleCheckboxClick, label: i18n('Group by family'), checked: groupByFamily }))) : ''),
            groupByFamily ? (_(FontListGrouped, { items: items })) : (_(FontList, { items: items }))));
    }
    ...
    function FontsPage() {
        return (_(Page, { title: i18n('Local fonts') },
            _(Fonts, null)));
    }
