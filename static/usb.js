(function (factory) {
    ...
    ***************************************************************************** */
    ...
    function UsbError(props) {
        var error = props.error;
        if (!error) {
            return null;
        }
        if (error.name === 'NotFoundError') {
            return (_(WarningMessage, null, i18n('USB device not found.')));
        }
        if (error.name === 'NotAllowedError') {
            return (_(WarningMessage, null, i18n('USB device is blocked.')));
        }
        return (_(WarningMessage, null, error.message));
    }
    ...
    var b = block('usb');
    function Usb() {
        var _a = h(null), usbDevice = _a[0], setUsbDevice = _a[1];
        var _b = h(null), error = _b[0], setError = _b[1];
        var handleClick = q(function () {
            navigator.usb.requestDevice({ filters: [] }).then(function (data) {
                setError(null);
                var result = {};
                var device = data;
        ...
        if (!isSsr && (!navigator.usb || typeof navigator.usb.requestDevice !== 'function')) {
            return (_(WarningMessage, null, i18n('WebUSB API is not supported.')));
        }
        return (_("div", { class: b() },
            _("div", { class: b('controls') },
                _(Button, { theme: "active", onClick: handleClick }, i18n('Select USB device'))),
            _(UsbError, { error: error }),
            usbDevice ? (_("div", { class: b('info') },
                _(TreeList, { data: usbDevice }))) : ''));
    }
