...
export function QuestionHdr() {
    ...
    const head = (<span>{i18n('Can I watch HDR video?')} <Result value={mainAnswer} /></span>);

    return (
        <ActiveQuestion head={head}>
            <ul>
                <li><QuestionHdrHead isHdr={Boolean(isHdr)} isVideoHdr={Boolean(isVideoHdr)} /></li>
...
export function QuestionHdrHead(props: QuestionHdrHeadProps) {
    ...
    if (!isHdr && isVideoHdr) {
        return (<span>{i18n('Has HDR support for video?')}<InfoLink href="https://developer.mozilla.org/en-US/docs/Web/CSS/@media/video-dynamic-range" />{'\u00a0'}<Result value={isVideoHdr} /></span>);
    }

    return (<span>{i18n('Is this an HDR-compatible screen?')}<InfoLink href="https://developer.mozilla.org/en-US/docs/Web/CSS/@media/dynamic-range" />{'\u00a0'}<Result value={isHdr} /></span>);
}
